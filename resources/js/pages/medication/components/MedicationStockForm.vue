...
import { ComboboxItem, RekaCombobox } from '@/components/ui/combobox';
...
const medicationOptions = computed<ComboboxItem[]>(() => {
    return (
        medications?.value?.data?.map((med) => ({
            label: med.trade_name ? `${med.trade_name} (${med.generic_name})` : med.generic_name,
            value: med.id,
        })) ?? []
    );
});
...
const cloneRow = (idx: number) => {
    const row = form.values.medications?.[idx];
    if (!row) return;
    push({ ...row });
};
...
const totalValue = computed(() =>
    (form.values.medications ?? []).reduce((acc, it) => acc + (Number(it?.quantity) || 0) * (Number(it?.buy_price) || 0), 0),
);
...
<template>
    <form id="medication-stock-form" class="flex flex-col gap-4" @submit="submit">
                        ...
                        <Field orientation="responsive" :data-invalid="errors.length > 0">
                            ...
                            <FieldContent>
                                <RekaCombobox
                                    ...
                                    @search="onMedicationSearch"
                                    v-model="form.values.medications[idx].medication_id"
                                    @update:model-value="
                                        (val: string | number) => {
                        ...
                        <Field orientation="responsive" :data-invalid="errors.length > 0">
                            ...
                            <FieldContent>
                                <DateOfBirthCalendar
                                    ...
                                    @selected="
                                        (val) => {
                                            form.setFieldValue(`medications[${idx}].expired_at` as any, val.toDate(getLocalTimeZone()).toISOString());
                                        }
                                    "
                                />
                            </FieldContent>
        ...
        <div class="sticky bottom-0 z-10 mt-3 flex items-center justify-between border-t py-3 backdrop-blur-sm">
            <div class="text-sm text-muted-foreground">
                Total: <span class="font-semibold">{{ totalQuantity }}</span> items —
